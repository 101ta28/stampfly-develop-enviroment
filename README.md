...
### 1. オフラインキャッシュの事前作成（オンライン環境で実行）

```bash
./create_offline_cache.sh
```

このスクリプトにより：

- PlatformIOの全依存関係がリポジトリ内の`.platformio`ディレクトリにダウンロードされます
- `platformio_cache.tar.gz`アーカイブが作成されます
...
- `platformio_cache.tar.gz`から依存関係が復元されます
- 完全オフライン環境でビルドが可能になります
